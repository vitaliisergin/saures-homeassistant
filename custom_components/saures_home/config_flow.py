...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not stats.get("sid_valid"):
            raise InvalidAuth("Authentication failed")

        if not objects:
            _LOGGER.warning("Saures account has no objects yet, continuing setup")
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            # Validate update interval according to API documentation
            # Minimum 60 minutes to prevent API rate limiting and IP bans
            update_interval = user_input.get("update_interval", DEFAULT_UPDATE_INTERVAL_MINUTES)
            if update_interval < MIN_UPDATE_INTERVAL:
                errors["update_interval"] = "interval_too_low"
                _LOGGER.warning(
                    "Update interval %d minutes is below minimum %d minutes. "
                    "API may block frequent requests.",
                    update_interval, MIN_UPDATE_INTERVAL
                )
            elif update_interval > MAX_UPDATE_INTERVAL:
                errors["update_interval"] = "interval_too_high"
